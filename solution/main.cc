...
int main() {
  ...
  // there are a handful of ways to do this but I felt a while(true) loop
  // would do the job with the proper break conditionals to get out of the loop
  while (true) {
    ...
    std::cin >> input;
    // Then this function would be where we get the new input and change
    // how the grid will look in the background
    PlayerInput(game_board, playerMoves, moveOptions, input, player);
    // These will be our break statements because without them we would
    ...
    if (PlayerWonCheck(game_board, player)) {
      ...
      break;
      // Here is a conditional if the entire board is filled, hence a tie
    } else if (moveOptions.size() == 0) {
      Display(game_board, moveOptions, 0);
      std::cout << "NO WINNERS: TIE" << std::endl;
      break;
    }
    // These if statements are just to swap the player once their turn is over.
    if (player == 1) {
      player = 2;
    } else {
      player = 1;
